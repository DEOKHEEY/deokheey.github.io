...
FULL SCAN 과 ROWID SCAN으로 2가지 뿐이다. 
...
rowid는 물리적인 row 주소값이다. RDMS에다가 row를 입력하면, row마다 고유한 주소를 가지고 있다. 그 주소가 rowid이다. rowid는 가상 컬럼이자 책 매커니즘에서의 쪽 번호라 생각하면 이해하기 편하다. 
...
특정 row 를 찾아가는 가장 빠른 방법은 rowid로 검색하는 것이다.(하나의 i/o만 발생) 하지만, rowid의 18자리를 기억하고 찾는 것은 힘들다. 그래서, index mechanism 을 사용해서 찾아 간다.
