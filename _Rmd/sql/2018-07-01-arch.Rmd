...
ORACLE->{'MEMORY (Instance)' 'DISK (Database)'}
}")
...
sequenceDiagram
    participant Client
    participant Server(Cursor)
    participant Shared pool
    Participant Data buffer cache
    participant Disk

    Client->>Server(Cursor): SQL문 전달
    Note left of Client: Sementic/Previlige 확인
    
    Alt Shared Pool에 dictionary가 있을 시
    Server(Cursor)->>Shared pool: user, table, col 등 확인
...
sequenceDiagram
    ...
    Client->>Server(Cursor): SQL문 전달
    Note left of Client: Soft Parsing
    
    Alt 동일 SQL문이 있을 시
    Server(Cursor)->>Shared pool: 동일 SQL문 확인
    Note right of Shared pool: LCO 레벨 정리: Hash f(x)에서 Hash value를 찾아 수행
    LCO->>Server(Cursor): LCO Sharing
    
    else 없을 시
    NOTE left of LCO: Hard parsing 유도
...
sequenceDiagram
    ...
    Note left of Client: Hard Parsing
    
    Opt Soft Parsing이 안될 경우
    Server(Cursor)->>Shared pool: library cache에 메모리 할당, 실행계획 생성
    Note over Server(Cursor),Shared pool: Library cache obj(LCO) 생성
...
sequenceDiagram
  ...
  Client->>Server(Cursor): SQL문 전달
  Server(Cursor)->>Data buffer cache: 실행계획에 따라 해당 block을 찾음
  Note left of Client: Fetch

  Alt Data buffer cache에 block이 없을 시
  Disk->>Data buffer cache: block을 memory단으로 올림

  else Data buffer cache에 block이 있을 시
  Data buffer cache->>Server(Cursor): 해당 block 속의 데이터를 추출해 Row단위로 Active Set 생성
  
  end

  opt Index scan일 경우(B * Tree 구조)
  Server(Cursor)->>Data buffer cache: Root block DBA로 Root block 찾음
  Data buffer cache->>Data buffer cache: branch, leaf block을 찾음
  Data buffer cache->>Server(Cursor): leaf block의 rowid값으로 데이터 추출 후 Active Set 생성
